...
const fs = require("fs");
const Database$1 = eval("require")("better-sqlite3");
const DATA_DIR = electron.app.isPackaged ? path.join(path.dirname(process.execPath), "data") : path.join(electron.app.getAppPath(), "data");
const DB_PATH = path.join(DATA_DIR, "timer.db");
const PREV_DATA_DIR = path.join(electron.app.getPath("userData"), "data");
...
async function initDatabase() {
  ...
  if (fs.existsSync(PREV_DB_PATH) && !fs.existsSync(MIGRATED_MARKER)) {
    ...
    try {
      fs.writeFileSync(MIGRATED_MARKER, (/* @__PURE__ */ new Date()).toISOString());
    } catch {
    }
...
function updateEvent(id, updates) {
  ...
  if (updates.lunarAnchor !== void 0) {
    fields.push("lunarAnchor = ?");
    values.push(updates.lunarAnchor ? JSON.stringify(updates.lunarAnchor) : null);
  }
...
function toggleCompletion(eventId, date) {
  const row = db.prepare("SELECT completedDates FROM events WHERE id = ?").get(eventId);
  if (!row) return false;
  let completedDates = [];
  if (row.completedDates) {
    try {
      completedDates = JSON.parse(row.completedDates);
    } catch {
      completedDates = [];
    }
  }
  ...
  ).run(JSON.stringify(completedDates), (/* @__PURE__ */ new Date()).toISOString(), eventId);
  return idx < 0;
}
function markReminderTriggered(reminderId) {
  db.prepare(
    "UPDATE reminders SET triggeredAt = ? WHERE id = ?"
  ).run((/* @__PURE__ */ new Date()).toISOString(), reminderId);
}
...
class HijriMonth extends MonthUnit {
  static NAMES = ["穆哈兰姆月", "色法尔月", "赖比尔·敖外鲁月", "赖比尔·阿色尼月", "主马达·敖外鲁月", "主马达·阿色尼月", "赖哲卜月", "舍尔邦月", "赖买丹月", "闪瓦鲁月", "都尔喀尔德月", "都尔黑哲月"];
  constructor(year, month) {
...
function solarToLunarDisplay(year, month, day) {
  ...
  return {
    ...
    // "乙未"
    zodiac: ZODIAC_NAMES[(lunar.getYear() - 4) % 12],
    // "马"
    solarTerm: termMap[dateKey] || void 0,
    // "小暑" if this day is a 节气
...
function extractFestivals(lunarDay, solarDay) {
  const festivals = [];
...
function expandOccurrences(event, range) {
  ...
  switch (rule.type) {
    case "daily":
      expandDaily(rule, anchor, range, event, out);
      break;
    case "weekly":
      expandWeekly(rule, anchor, range, event, out);
      break;
    case "monthly":
      expandMonthly(rule, anchor, range, event, out);
      break;
    case "yearly":
      expandYearly(rule, anchor, range, event, out);
      break;
    case "lunar-yearly":
      expandLunarYearly(rule, event, range, out);
      break;
  }
  return out;
}
function expandDaily(rule, anchor, range, event, out) {
...
function expandWeekly(rule, anchor, range, event, out) {
  const interval = rule.interval || 1;
...
function expandMonthly(rule, anchor, range, event, out) {
  const interval = rule.interval || 1;
...
function expandYearly(rule, anchor, range, event, out) {
  const interval = rule.interval || 1;
  ...
  while (curYear <= rangeEndYear) {
    ...
    if (d.isBefore(dayjs(range.start))) {
      curYear += interval;
      continue;
    }
...
function expandLunarYearly(rule, event, range, out) {
  const interval = rule.interval || 1;
...
function shouldEnd(rule, anchor, current, count) {
...
function checkReminders() {
  ...
  for (const raw of rawEvents) {
    const reminders = (raw.reminders || []).filter((r) => r.enabled);
...
const PROGRAM_DIR = electron.app.isPackaged ? path.dirname(process.execPath) : electron.app.getAppPath();
const BACKUP_DIR = path.join(PROGRAM_DIR, "backups");
const OLD_BACKUP_DIR = path.join(electron.app.getPath("userData"), "backups");
...
let retryTimer = null;
function migrateBackupsFromOldLocation() {
  try {
    if (!fs.existsSync(OLD_BACKUP_DIR)) return;
    const oldFiles = fs.readdirSync(OLD_BACKUP_DIR).filter((f) => (f.startsWith(BACKUP_PREFIX) || f.startsWith(OLD_BACKUP_PREFIX)) && f.endsWith(".db"));
...
function rotateBackups() {
  ...
  for (let w = 1; w <= WEEKLY_KEEP_WEEKS; w++) {
    ...
    const weekBackups = files.filter(
      (f) => f.date.isAfter(weekStart) || f.date.isSame(weekStart, "day") && (f.date.isBefore(weekEnd) || f.date.isSame(weekEnd, "day"))
    );
  ...
  for (let m = 1; m <= MONTHLY_KEEP_MONTHS; m++) {
    ...
    const monthBackups = files.filter(
      (f) => f.date.isAfter(monthStart) || f.date.isSame(monthStart, "day") && (f.date.isBefore(monthEnd) || f.date.isSame(monthEnd, "day"))
    );
...
function listBackupFilesRaw() {
  ...
  return files.sort((a, b) => b.date.unix() - a.date.unix());
}
function classifyTier(backupDate) {
  const today = dayjs();
...
function exportToJson() {
  ...
  const envelope = {
    ...
    appVersion: electron.app.getVersion() || "0.1.0",
    events
  };
  return JSON.stringify(envelope, null, 2);
}
function importFromJson(jsonContent, mode = "merge-overwrite") {
  let data;
  try {
  ...
  for (let i = 0; i < data.events.length; i++) {
    ...
    if (!ev.id) {
      errors.push(`Event #${i + 1}: missing id`);
      continue;
    }
    if (!ev.title) {
    ...
    if (seenIds.has(ev.id)) {
      errors.push(`Event #${i + 1}: duplicate id "${ev.id}"`);
      continue;
    }
    seenIds.add(ev.id);
  }
  ...
  let skippedCount = 0;
  if (mode === "replace") {
    db2.prepare("DELETE FROM reminders").run();
  ...
  const importTransaction = db2.transaction(() => {
    for (const ev of data.events) {
      if (mode === "merge") {
        const existing = db2.prepare("SELECT id FROM events WHERE id = ?").get(ev.id);
        if (existing) {
          skippedCount++;
          continue;
        }
  ...
  try {
    ...
    return { success: false, importedCount: 0, skippedCount: 0, errors: [`Import transaction failed: ${e.message}`] };
  }
...
function restoreFromBackup(backupFilename) {
  ...
  try {
    ...
    return { success: false, error: `Backup file is not a valid SQLite database: ${e.message}` };
  }
  if (!verifyBackupIntegrity(backupPath)) {
    return { success: false, error: "Backup integrity check failed" };
  }
  ...
  try {
    ...
    return { success: false, error: `Failed to create pre-restore backup: ${e.message}` };
  }
  setIsRestoring(true);
  try {
    const db2 = getDatabase();
...
function registerIpcHandlers() {
  electron.ipcMain.handle("events:query", async (_e, rangeStart, rangeEnd) => {
    const rawEvents = queryAllEvents();
  ...
  electron.ipcMain.handle("events:create", async (_e, eventData) => {
    return createEvent(eventData);
  });
  electron.ipcMain.handle("events:update", async (_e, id, updates) => {
    updateEvent(id, updates);
  });
  electron.ipcMain.handle("events:delete", async (_e, id) => {
    deleteEvent(id);
  });
  electron.ipcMain.handle("events:toggleCompletion", async (_e, eventId, date) => {
    return toggleCompletion(eventId, date);
  });
  electron.ipcMain.handle("lunar:range", async (_e, rangeStart, rangeEnd) => {
    const result = {};
  ...
  electron.ipcMain.handle("lunar:day", async (_e, year, month, day) => {
    return solarToLunarDisplay(year, month, day);
  });
  electron.ipcMain.handle("backup:status", async () => {
    return getBackupStatus();
  });
  electron.ipcMain.handle("backup:list", async () => {
    return listBackups();
  });
  electron.ipcMain.handle("backup:trigger", async () => {
    return performBackup();
  });
  electron.ipcMain.handle("backup:export-json", async () => {
  ...
  electron.ipcMain.handle("backup:pick-import-file", async () => {
  ...
  electron.ipcMain.handle("backup:import-json", async (_e, filePath, mode) => {
    if (getIsRestoring()) {
      return { success: false, importedCount: 0, skippedCount: 0, errors: ["Database is being restored, please wait"] };
    }
    const content = fs.readFileSync(filePath, "utf-8");
    return importFromJson(content, mode);
  });
  electron.ipcMain.handle("backup:restore", async (_e, backupFilename) => {
    return restoreFromBackup(backupFilename);
  });
}
function mapRawToEvent(raw) {
  return {
    id: raw.id,
    title: raw.title,
...
function createMainWindow() {
  const appIcon = electron.nativeImage.createFromPath(path.join(__dirname, "../../resources/icon.png"));
...
function createTray() {
  try {
    ...
    if (fs.existsSync(iconPath)) {
      normalTrayIcon = electron.nativeImage.createFromPath(iconPath).resize({ width: 16, height: 16 });
    } else {
  ...
  tray = new electron.Tray(normalTrayIcon);
...
function createPlaceholderIcon() {
  ...
  return electron.nativeImage.createFromBuffer(buf, { width: w, height: h });
}
...
function showPopupNearTray() {
  ...
  let x = Math.round(trayBounds.x + trayBounds.width / 2 - 140);
